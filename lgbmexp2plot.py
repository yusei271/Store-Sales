...
# インデックスを調整して、全データではなく一部のみを可視化
sample_size = 1000  # 可視化するデータ数を減らす
indices = np.random.choice(len(y_valid_orig), sample_size, replace=False)

# 可視化
plt.figure(figsize=(12, 6))
plt.scatter(indices, y_valid_orig.iloc[indices], label="Actual Sales", alpha=0.6, color='blue', s=10)
plt.scatter(indices, y_pred[indices], label="Predicted Sales", alpha=0.6, color='orange', s=10)

plt.xlabel("Sample Index")
plt.ylabel("Sales")
plt.title("Actual vs Predicted Sales (Sampled)")
plt.legend()
plt.show()