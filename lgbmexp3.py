...
# 📌 `lag` 特徴量を `df_test` に追加
for lag in [7, 14, 30]:
    last_sales = df_train.groupby("store_nbr")["sales"].last()  # 各店舗の最新 `sales`
    df_test[f"lag_{lag}"] = df_test["store_nbr"].map(last_sales)  # `df_test` に適用

# 📌 `rolling_mean` を `df_test` に追加
for window in [7, 14, 30]:
    rolling_sales = df_train.groupby("store_nbr")["sales"].rolling(window=window, min_periods=1).mean().groupby(level=0).last()
    df_test[f"rolling_mean_{window}"] = df_test["store_nbr"].map(rolling_sales)
...
# 1. sample_submission.csv を読み込む
df_submission = pd.read_csv("sample_submission.csv")

# 2. 予測値を入力（log1p を元に戻す）
df_submission["sales"] = np.expm1(model.predict(df_test[features]))

# 3. 負の値を 0 に補正
df_submission["sales"] = df_submission["sales"].clip(0)

# 4. CSV として保存
df_submission.to_csv("submission.csv", index=False)

print("✅ Kaggle 提出用ファイル 'submission.csv' を作成しました！")